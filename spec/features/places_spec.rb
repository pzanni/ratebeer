...
describe "Places" do
  ...
  it "if several is returned by the API, they are shown at the page" do
  	allow(BeermappingApi).to receive(:places_in).with("kallio").and_return(
  		[Place.new(name:"Relaxin", id: 1), Place.new(name: "Iltakoulu", id: 1)], 
  	)
  	...
  	expect(page).to have_content "Relaxin"
  	expect(page).to have_content "Iltakoulu"
  end

  it "if none is returned by the API, a message is shown at the page" do
  	visit places_path
  	fill_in('city', with: 'kallio')
  	click_button "Search"
